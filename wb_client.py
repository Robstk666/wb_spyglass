...
def _basket_urls(sku: int) -> list[str]:
    ...
    WB постоянно меняет роутинг, поэтому возвращаем сразу весь массив корзин
    начиная с вероятной (чтобы 200 OK пришел мгновенно), и затем все остальные,
    чтобы на 100% исключить ложные 404 (которые ломают парсер).
    """
    vol  = sku // 100_000
    part = sku // 1_000
